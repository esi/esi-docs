...
Every route named on this page can be tried in the [API Explorer](/api-explorer).
...
That is the tidy end of the format. Here is the messy end: the client's own location label, read out of the running UI by [Sanderling](https://github.com/Arcitectus/Sanderling).
...
Which language ends up on which side depends on the client that produced the text, and the captured samples disagree, so do not rely on one being English.
...
| Named | `yellow` | A small fixed set of names |
...
Alpha is usually `ff`, but not always — the client's own default body text is `#bfffffff`, white at 75% — so dropping it is a simplification rather than a rule.
[PodMail](https://github.com/zKillboard/podmail) drops a leading `ff` and uses the remaining `#RRGGBB` directly, which is a fair trade as long as you accept that anything semi-transparent comes out wrong.

For named colors, these are the values [RIFT](../community/rift-intel-fusion-tool/index.md) resolves:

| Name | Value | Name | Value | Name | Value |
| ---- | ----- | ---- | ----- | ---- | ----- |
| `black` | `0xff000000` | `navy` | `0xff000080` | `orange` | `0xffff8000` |
| `silver` | `0xffc0c0c0` | `blue` | `0xff0000ff` | `lightred` | `0xffcc3333` |
| `gray` / `grey` | `0xff808080` | `purple` | `0xff800080` | `lightblue` | `0xff7777ff` |
| `white` | `0xffffffff` | `teal` | `0xff008080` | `lightgreen` | `0xff80ff80` |
| `maroon` | `0xff800000` | `fuchsia` | `0xffff00ff` | `transparent` | `0x00000000` |
| `red` | `0xffff0000` | `aqua` | `0xff00ffff` | `green` | `0xff008000` |
| `lime` | `0xff00ff00` | `olive` | `0xff808000` | `yellow` | `0xffffff00` |

Do not pass these names to CSS.
`orange` is not CSS's `orange` — `0xffff8000` against `#ffa500` — and `lightblue` and `lightgreen` are worse, because CSS knows both names and will quietly paint `#add8e6` and `#90ee90` instead of EVE's `#7777ff` and `#80ff80`.
Only `lightred` is not a CSS name at all. Resolve names through the table instead.
...
!!! tip "Colors assume a dark background"
    ...
    Either render on a dark surface — PodMail does, and lightens any text whose contrast against its near-black background falls below 4.5:1, telling the reader it did so — or map the colors onto your own palette before rendering.
...
Others have been reported but are not confirmed here: `hyperNet:` for a HyperNet Relay offer, with a UUID payload, and `shipSkinDesign:` alongside `shipSkinListing:`.
...
Three open-source renderers worth reading before writing your own:

- [RIFT](../community/rift-intel-fusion-tool/index.md) ([source](https://gitlab.com/rift-intel-fusion-tool/rift-intel-fusion-tool/-/blob/release/src/main/kotlin/dev/nohus/rift/compose/text/ParseEveFormattedTextUseCase.kt)) — Kotlin. A tolerant tokenizer, a separate nesting pass, and the named-color table above.
- [JitaSpace](../community/jitaspace/index.md) ([source](https://github.com/joaomlneto/jitaspace/blob/main/packages/tiptap-eve/Extensions/EveLink.ts)) — TypeScript. Renders and composes mail in a TipTap editor, with the EVE schemes registered as custom protocols.
- [PodMail](https://github.com/zKillboard/podmail) ([source](https://github.com/zKillboard/podmail/blob/master/js/app.js)) — JavaScript. Rewrites in-game links to web equivalents, sanitizes with DOMPurify, and lightens low-contrast text against its own near-black background.

## What we still don't know

- The full list of URI schemes. Schemes are payloads the UI passes on rather than something the parser knows, so there is no list to read off.
- The meaning of some payload segments: the second segment of `recruitmentAd:` and the three parts of `careerProgramNode:`.
- How color values are parsed. The three forms above are what appears in real data and what third-party parsers accept, but the code that reads them lives in the client's Python UI, which is not public.
- What consumes `<loc>`, and whether it can carry attributes.

If you can fill any of these in — ideally with a captured example — please contribute.
